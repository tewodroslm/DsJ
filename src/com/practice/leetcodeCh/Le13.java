...
//        String s = "D";    // 500
    ...
    public static int romanToInt(String s){
        ...
        if(att.length == 1){
            return symVal.get(att[0]);
        }
        int ans = 0;
        for(int i = 0; i < att.length-1; i++){
            if(symVal.get(att[i]) > symVal.get(att[i+1])){
                ans += symVal.get(att[i]);
            }else if(symVal.get(att[i]) < symVal.get(att[i+1])){
                ans = ans + (symVal.get(att[i+1]) - symVal.get(att[i]));
                i++;
            }else if(symVal.get(att[i]) == symVal.get(att[i+1])){
                ans += symVal.get(att[i]);
            }
        ...
        if(att.length > 1){
            if(symVal.get(att[att.length-1]) <= symVal.get(att[att.length-2])){
                ans += symVal.get(att[att.length-1]);
            }
        }
